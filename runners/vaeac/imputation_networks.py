...
def get_imputation_networks(one_hot_max_sizes, width=256, depth=10, latent_dim=64):
    """
    ...
    # Prior network
    prior_layers = [
        # PrintLayer('Prior network pre-onehot' + str(one_hot_max_sizes + [0] * len(one_hot_max_sizes))),
        CategoricalToOneHotLayer(one_hot_max_sizes +
                                 [0] * len(one_hot_max_sizes)),
        # PrintLayer('Prior network'),
        MemoryLayer('#input'),
        nn.Linear(sum(max(1, x) for x in one_hot_max_sizes) +
    ...
    generative_layers.extend([
        ...
        # PrintLayer('Generative network output'),
    ])
